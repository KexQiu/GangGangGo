...
describe('team query cache', () => {
  it('updates the team and invalidates snapshots after a member mutation', async () => {
    ...
    queryClient.setQueryData<TeamSnapshotsResponse>(teamQueryKeys.teamSnapshots(userId), {
      date: '2026-07-13',
      snapshots: [],
    });
    ...
    expect(queryClient.getQueryData(teamQueryKeys.team(userId))).toEqual(response);
    expect(queryClient.getQueryState(teamQueryKeys.teamSnapshots(userId))?.isInvalidated).toBe(true);
  });
  ...
  it('removes snapshots after leaving the team', async () => {
    ...
    expect(queryClient.getQueryData(teamQueryKeys.team(userId))).toEqual({ team: null });
    expect(queryClient.getQueryData(teamQueryKeys.teamSnapshots(userId))).toBeUndefined();
  });
});
