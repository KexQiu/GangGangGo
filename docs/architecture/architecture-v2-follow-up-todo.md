...
- [ ] 明确共享快照、报告和 Push 的独立失败状态与重试入口。
  - 验收：一次本地操作风暴最多产生一个运行中同步和一个尾随同步；任一子任务失败不阻塞其他子任务。
  - 证据：2026-07-13 新增纯调度核心及 4 项自动化测试，覆盖防抖、single-flight、尾随补跑、前后台和失败隔离。
...
- [ ] 提醒：聚焦刷新、后台停刷、取消请求和游标分页。
  ...
  - 进展：2026-07-13 已增加 API transport、同步协调器、报告构建、分页、迁移、提醒模型、轮询条件、小队缓存和认证偏好迁移测试，移动端测试增至 29 项。
