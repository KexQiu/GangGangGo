...
- [x] 为同步风暴和 Watch 后台行为建立可重复的计数型测试。
  - 证据：Drizzle 客户端支持注入仅测试使用的查询计数器；Postgres CI 断言空/5 条提醒时收件箱均固定 1 条 SQL、会话详情均固定 3 条 SQL，90 天 bulk upsert 固定 1 条 SQL。SyncCoordinator 测试统计一次运行中任务和一次尾随补跑；Watch 退避策略测试统计前台延迟序列并断言后台不产生调度。
