...
- [ ] 覆盖用户 upsert、权益、共享权限和日期范围过滤。
  - 进展：已新增 11 项真实 Postgres 用例，覆盖上述认证、小队、邀请、提醒、回执、报告、用户、权益和共享场景；高级报告响应已由单一 90 天汇总改为服务端计算 7/30/90 天三档汇总，重复回执不再消耗修改次数。本机 Docker Hub 镜像拉取受阻，需以 Draft PR 的 PostgreSQL 17 CI 结果完成验收后再勾选。
...
- [x] 为同步风暴和 Watch 后台行为建立可重复的计数型测试。
  - 进展：Drizzle 客户端已支持注入仅测试使用的查询计数器；Postgres 用例断言收件箱固定 1 条 SQL、会话详情固定 3 条 SQL、90 天 bulk upsert 固定 1 条 SQL，待 CI 实际执行确认。SyncCoordinator 测试统计一次运行中任务和一次尾随补跑；Watch 退避策略测试统计前台延迟序列并断言后台不产生调度。
