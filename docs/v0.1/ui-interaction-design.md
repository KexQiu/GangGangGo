...
1. 主标题和说明文字。
2. 蹲会儿优先行动卡，使用明确的“开始计时”按钮，提示及时收工。
3. 未开启提醒时的小暗号引导。
4. 菊花抬紧凑入口，展示当日进度并进入训练模式。
5. 今日小账本快速打卡。

页头规则：

- 所有页面仅显示一个主标题和可选说明文字，不在主标题上方显示重复的小标题。
- 二级流程继续保留 `AppTopBar` 返回或关闭操作。

Tab 状态规则：

- 选中项使用深绿或亮绿，并在图标背后显示浅绿、固定尺寸的状态底座。
- 未选中项使用中性灰；状态底座与标签颜色共同表达当前页，不能只依赖颜色。
